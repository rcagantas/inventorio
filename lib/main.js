...
class StateManager extends State<StateManagerWidget> {
  ...
  void addProductImage(Product product, File file) async {
    ...
    setState(() {
      _imageMap[product.code] = file;
    });
  }

  void _uploadImage(String refCode, File file) async {
    StorageReference ref = FirebaseStorage.instance.ref().child(refCode);
    try {
      print('Storage reference $ref');
      StorageUploadTask uploadTask = ref.put(file, const StorageMetadata(contentLanguage: "en"));
...
class InventoryItemTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return new Dismissible(
      ...
      child: new Row(
        children: <Widget>[
          new SquareImage(imageFile: state.getImage(product),),
          new Expanded(
            flex: 2,
            child: new Column(
              children: <Widget>[
                ...
                child: new Center(
                  child: new FlatButton(
                    ...
                    child: new SquareImage(
                      side: 250.0,
                      imageFile: state.getImage(product),
                    ),
                  )
                ),
              ),
